...
export default {
  ...
  methods: {
    ...
    handleMouseDownOnScale: function(type, event) {
      ...
      // 广播事件
      bus.$emit("XPE/scale/guides/drag/start", {
        // 参考线key
        key: key,
        // 参考线类别
        type: type,
        ...
        list: [
          {
            ...
            text: _t.guidesMap[_t.currentProject]["showGuides"]
              ? "隐藏参考线"
              : "显示参考线",
            enable: true,
            action: {
          ...
          },
          {
            ...
            text: _t.guidesMap[_t.currentProject]["showToolTip"]
              ? "隐藏参考线坐标"
              : "显示参考线坐标",
            enable: true,
            action: {
      ...
      Object.keys(guidesMap).map(type => {
        Object.keys(guidesMap[type]).map(key => {
          guidesMap[type][key]["toolTip"]["style"] = {
            ...guidesMap[type][key]["toolTip"]["style"],
            display: _t.guidesMap[_t.currentProject]["showToolTip"]
              ? "inline-block"
              : "none"
          };
        });
  ...
  created: function() {
    ...
    bus.$on("XPE/scale/guides/drag/stop", function(info) {
    ...
    // 清空编辑器画板
    bus.$on("XPE/board/clear", function() {
      // 清空所有参考线
      _t.guidesMap = {};
    });
    // 清空当前项目画布
    bus.$on("XPE/canvas/clear", function(projectID) {
      // 清空当前项目的参考线
      _t.clearGuidesByProject(projectID);
    });
    // 清空当前项目的参考线
    bus.$on("XPE/scale/guides/clear", function(projectID) {
      _t.clearGuidesByProject(projectID);
    });
    bus.$on("XPE/scale/guides/toggle", function() {
      if (!Object.keys(_t.guidesMap).length || !_t.currentProject) {
        console.warn("Notice::请先创建项目！");
        return;
      }
      _t.guidesMap[_t.currentProject]["showGuides"] = !_t.guidesMap[
        _t.currentProject
      ]["showGuides"];
    });
    bus.$on("XPE/scale/guides/toolTip/toggle", function() {
      if (!Object.keys(_t.guidesMap).length || !_t.currentProject) {
        console.warn("Notice::请先创建项目！");
        return;
      }
      _t.guidesMap[_t.currentProject]["showToolTip"] = !_t.guidesMap[
        _t.currentProject
      ]["showToolTip"];
      // 处理toolTip显示隐藏
      _t.toggleToolTip();
    });
    bus.$on("XPE/project/add/ok", function(projectInfo) {
      _t.guidesMap = {
        ..._t.guidesMap,
        [projectInfo.id]: {
          x: {},
    ...
    bus.$on("XPE/project/trigger", function(projectID) {
      // 更新当前激活项目
      _t.currentProject = projectID;
    });
    bus.$on("XPE/project/remove", function(projectID) {
      // 删除当前操作对象数据
